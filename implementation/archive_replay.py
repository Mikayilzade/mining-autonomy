...
def archive_observation_items(archive: EvidenceArchive, *, now: datetime | None = None,
    ...
    Testnet/unknown entries are never admitted. Even fresh production evidence remains
    HOLD-only because archive entries deliberately lack executable raw task/service
    payloads and trusted policy/cost context.
    """
    ...
    for entry in _latest_production_entries(archive):
        ...
        items.append(ObservationItem(
            source_type="archived_evidence",
            platform=entry.platform,
            external_id=entry.bundle_sha256,
            state="hold",
            expected_monthly_value_usd=None,
            expected_margin_usd=None,
            reasons=tuple(reasons),
            demand_evidence_class=_evidence_class(entry),
...
def archive_replay_report(archive: EvidenceArchive, *, now: datetime | None = None,
    ...
    return {
        ...
        "production_platform_count": len(rows),
        ...
        "platforms": rows,
        "archive_evidence_can_authorize_action": False,
