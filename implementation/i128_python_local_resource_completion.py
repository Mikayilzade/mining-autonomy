"""I128 complete no-spend python_local resource-evidence assembly path.

Builds on I127 by resolving the remaining local quota/rate *interface semantics*
without claiming infinite host capacity. It combines:
1) the fixed I056/I053 local probe for availability, programmatic access, latency,
   reliability, quality and measured max_parallelism;
2) I126 exact python_local intrinsic configuration invariants;
3) a python_local-only local-interface semantic probe establishing that this
   repository-local executor has no external provider quota/rate-limit layer
   (`None` means not applicable at that interface, not unlimited compute);
4) optional measured energy + explicit tariff through the existing I054 adapter.

No market/network request, credentials, paid infrastructure, task acceptance,
submission, authorization creation, spend, or value movement occurs here. Without
real energy input, electricity remains the single resource-evidence gap.
"""
...
SCHEMA = "mining-autonomy/i128-python-local-resource-completion/v1"
...
def build_local_interface_semantic_evidence(
    ...
    `None` means no external provider/interface quota or rate-limit primitive for
    the repository-local deterministic executor. Host capacity remains bounded by
    measured max_parallelism/latency/reliability and by economics/opportunity cost.
    """
    ...
    return tuple(
        make_evidence(
            evidence_id=f"i128-python-local-interface-{parameter}",
            backend_id="python_local",
            parameter=parameter,
            value=value,
            source_kind="system_probe",
            source_ref=f"i128-interface-semantics:{digest}:{parameter}",
...
def run_no_spend_bundle(
    ...
    *,
    ...
    if energy_kwh_per_task is not None and (
        energy_kwh_per_task < 0 or float(tariff_usd_per_kwh) < 0
    ):
    ...
    if energy_kwh_per_task is not None and tariff_usd_per_kwh is not None:
        energy = EnergyMeasurement(
            ...
            source_ref="i128:explicit-local-energy-measurement",
            source_content_digest=_digest({
                "schema": "mining-autonomy/i128-explicit-energy-input/v1",
                "energy_kwh_per_task": float(energy_kwh_per_task),
                "tariff_usd_per_kwh": float(tariff_usd_per_kwh),
                "observed_at": observed_at,
            }),
            notes="Caller-supplied measured energy and explicit tariff; I128 does not infer either value.",
        )
...
def main(argv=None) -> int:
    ...
    parser.add_argument("--output", default="I128_PYTHON_LOCAL_RESOURCE_COMPLETION_RESULT.json")
