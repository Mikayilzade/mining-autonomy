...
def _positive_optional_number(value: Any, *, field: str) -> Optional[float]:
    if value is None:
        return None
    number = _finite_number(value, field=field)
    if number <= 0:
...
def _nonnegative_optional_number(value: Any, *, field: str) -> Optional[float]:
    if value is None:
        return None
    return _nonnegative_number(value, field=field)


def _probability(value: Any, *, field: str) -> float:
    number = _finite_number(value, field=field)
    if not 0.0 <= number <= 1.0:
        raise ValueError(f"{field}_must_be_probability")
    return number
...
class ExecutionBackend:
    ...
    def marginal_cost_usd(self) -> float:
        units = _nonnegative_number(self.units_per_task, field="units_per_task")
        marginal_unit = _nonnegative_number(self.marginal_cost_per_unit_usd, field="marginal_cost_per_unit_usd")
        electricity = _nonnegative_number(self.electricity_per_task_usd, field="electricity_per_task_usd")
        external_api = _nonnegative_number(self.external_api_per_task_usd, field="external_api_per_task_usd")
        retry = _nonnegative_number(self.retry_failure_expected_cost_usd, field="retry_failure_expected_cost_usd")
        maintenance_minutes = _nonnegative_number(self.maintenance_minutes_per_task, field="maintenance_minutes_per_task")
        human_hourly = _nonnegative_number(self.human_time_value_per_hour_usd, field="human_time_value_per_hour_usd")
        opportunity = _nonnegative_number(self.opportunity_cost_per_task_usd, field="opportunity_cost_per_task_usd")
    ...
    def allocated_fixed_cost_per_task_usd(self) -> Optional[float]:
        fixed = _nonnegative_number(self.fixed_monthly_cost_usd, field="fixed_monthly_cost_usd")
        if self.sunk_or_already_committed:
            return 0.0
        if fixed == 0:
            return 0.0
        basis = _positive_optional_number(self.allocation_basis_tasks_per_month, field="allocation_basis_tasks_per_month")
        if basis is None:
            return None
        value = fixed / basis
    ...
    def effective_success_probability(self) -> float:
        reliability = _probability(self.reliability_probability, field="reliability_probability")
        quality = _probability(self.quality_probability, field="quality_probability")
        result = _finite_product(reliability, quality, field="success_probability")
...
class TaskEconomics:
    ...
    def fixed_fees_usd(self) -> float:
        payout = _nonnegative_number(self.gross_payout_usd, field="gross_payout_usd")
        platform_fee = _nonnegative_number(self.platform_fee_usd, field="platform_fee_usd")
        platform_rate = _probability(self.platform_fee_rate, field="platform_fee_rate")
        transaction = _nonnegative_number(self.transaction_fee_usd, field="transaction_fee_usd")
        gas = _nonnegative_number(self.gas_fee_usd, field="gas_fee_usd")
        withdrawal = _nonnegative_number(self.withdrawal_conversion_fee_usd, field="withdrawal_conversion_fee_usd")
    ...
    def expected_collect_probability(self) -> float:
        acceptance = _probability(self.acceptance_probability, field="acceptance_probability")
        dispute = _probability(self.dispute_probability, field="dispute_probability")
        nonpayment = _probability(self.nonpayment_probability, field="nonpayment_probability")
        result = _finite_product(acceptance, 1.0 - dispute, field="collect_probability")
        result = _finite_product(result, 1.0 - nonpayment, field="collect_probability")
        return round(result, 6)
...
def quote_backend(task: TaskEconomics, backend: ExecutionBackend) -> BackendQuote:
    ...
    units_per_task = _nonnegative_number(backend.units_per_task, field="units_per_task")
    quota_remaining = _nonnegative_optional_number(backend.quota_units_remaining, field="quota_units_remaining")
    _nonnegative_optional_number(backend.quota_units_monthly, field="quota_units_monthly")
    ...
    minimum_success = _probability(task.minimum_success_probability, field="minimum_success_probability")
    ...
    gross_payout = _nonnegative_number(task.gross_payout_usd, field="gross_payout_usd")
    expected_revenue = _finite_product(gross_payout, success, field="expected_revenue")
    ...
    expected_margin_after_fixed = None if allocated_fixed is None else _finite_sum((expected_margin, -allocated_fixed), field="expected_margin_after_fixed")
    ...
    rate_limit = _nonnegative_optional_number(backend.rate_limit_per_minute, field="rate_limit_per_minute")
    ...
    hard_live_blockers = {"backend_not_currently_available", "credentials_required_before_live_execution", "paid_account_required_before_live_execution", "new_spend_requires_explicit_authorization", "fixed_cost_allocation_basis_unknown"}
    if planning_reasons:
        state = "hold"
    ...
    fixed_reference = _nonnegative_number(backend.fixed_monthly_cost_usd, field="fixed_monthly_cost_usd")
    return BackendQuote(backend.backend_id, state, tuple(dict.fromkeys(planning_reasons)), tuple(dict.fromkeys(live_blockers)), round(marginal,6), allocated_fixed, round(fixed_reference,6), success, round(expected_revenue,6), round(expected_margin,6), None if expected_margin_after_fixed is None else round(expected_margin_after_fixed,6), round(ratio,6), latency, backend.max_parallelism, rate_limit)
...
def validate_watcher_policy(policy: WatcherPolicy, *, platform_min_interval_seconds: int) -> dict[str, Any]:
    reasons: list[str] = []
    if policy.mode not in {"poll", "webhook", "websocket"}: reasons.append("unsupported_watcher_mode")
    if isinstance(policy.polling_interval_seconds, bool) or not isinstance(policy.polling_interval_seconds, int) or policy.polling_interval_seconds <= 0: reasons.append("invalid_poll_interval")
    if isinstance(platform_min_interval_seconds, bool) or not isinstance(platform_min_interval_seconds, int) or platform_min_interval_seconds < 0: reasons.append("invalid_platform_min_interval")
    elif policy.mode == "poll" and isinstance(policy.polling_interval_seconds, int) and not isinstance(policy.polling_interval_seconds, bool) and policy.polling_interval_seconds < platform_min_interval_seconds: reasons.append("polling_faster_than_platform_limit")
    if policy.llm_on_every_poll: reasons.append("llm_on_every_poll_disallowed_by_default")
    if not policy.local_filtering_required: reasons.append("local_filtering_required")
    if not policy.obey_platform_rate_limits or policy.bypass_product_limits: reasons.append("rate_limit_or_product_limit_bypass")
    if policy.network_enabled: reasons.append("network_must_remain_disabled_in_i048")
